...
#include "core/test/test_bound_entity_ring_queue.h"
...
typedef BoundEntityRingQueue<core::TestTarget> BoundEntityRingQueueImp;
static BoundEntityRingQueueImp *queue = NULL;
  ...
  while(true) {
    ...
    if(value < random) {
      for(; iterator < max_value; ++iterator) {
        ...
        if(target) {
          target->Set(iterator);
          while(queue->Add(target) != kErrorCodeSuccess) {
            usleep(Terminal::producer_sleep_);
          }
        }
      }
...
int main(int argc, char* argv[]) {
  ...
  if(argc == 5) {
    ...
    core::queue = new core::BoundEntityRingQueueImp(ring_size, blocking);
