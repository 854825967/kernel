...
// $Date$
//
// Define class TrainTownPeriodSorter.
//

#ifndef __TRAIN__TOWN__PERIOD__SORTER__H
#define __TRAIN__TOWN__PERIOD__SORTER__H

#include "include/IDatabaseSorter.h"
#include "db_server/client/SorterInterface.h"

class TrainTownPeriodSorter : public SorterInterface {
 public:
  TrainTownPeriodSorter() : sort_complete_(false), data_length_(0) {}
  virtual ~TrainTownPeriodSorter() {}
  ...
  virtual bool Sort(const char *data, size_t size) {
    if(size > MAX_BUFFER_SIZE || size % sizeof(SCenterDBChampionList_Get) != 0 ||
        size < sizeof(SCenterDBChampionList_Get)) {
      return false;
    } else {
...
#endif // __TRAIN__TOWN__PERIOD__SORTER__H
