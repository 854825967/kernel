...
extern char *(*sysstrdup)(const char*);
...
char *(*sysstrdup)(const char*)=strdup;
...
NEDMALLOCNOALIASATTR char*  nedstrdup(const char* str) THROWSPEC																{ return nedpstrdup2(str); }
...
NEDMALLOCNOALIASATTR char* nedpstrdup2 (const char * string)
{
	//MessageBox(0, "nedmalloc: can't use strdup()", "error", MB_OK);
	//abort();
	char *memory;
	size_t size = 0;
	...
	if (memory = nedmalloc(size))
	{
		strcpy_s(memory, size, string);
		return memory;
	}
